      ...
      <section className="outreach-dialog" role="dialog" aria-modal="true" aria-labelledby="outreach-dialog-title">
        ...
        {!loading && !campaign && preflight ? (
          <>
            ...
            {preflight.sample ? <details className="outreach-preview"><summary><MailCheck size={16} /> Пример первого письма</summary><div><small>{preflight.sample.company_name} · {preflight.sample.recipient}</small><strong>{preflight.sample.subject}</strong><EmailContentPreview html={preflight.sample.html_body} text={preflight.sample.body} />{preflight.sample.attachments?.length ? <p>Вложения: {preflight.sample.attachments.map((file) => file.filename).join(" · ")}</p> : null}</div></details> : null}
...
function CampaignProgress({ campaign, acting, onAction }: { campaign: OutreachCampaign; acting: boolean; onAction: (action: () => Promise<OutreachCampaign>) => Promise<void> }) {
  ...
  return (
    <div className="campaign-progress">
      <div className={`campaign-state campaign-state--${campaign.status}`}>{campaign.status === "completed" ? <CheckCircle2 size={20} /> : campaign.status === "paused" ? <Pause size={20} /> : campaign.status === "stopped" ? <Square size={20} /> : campaign.status === "cooldown" ? <Clock3 size={20} /> : campaign.status === "interrupted" ? <AlertTriangle size={20} /> : <LoaderCircle className="spin" size={20} />}<div><strong>{campaign.reply_wait_reason ? "Рассылка ожидает ответа клиенту" : campaignLabels[campaign.status]}</strong><small>{campaign.reply_wait_reason || campaign.pause_reason || `Следующее действие: ${formatDateTime(campaign.next_send_at)}`}</small></div></div>
