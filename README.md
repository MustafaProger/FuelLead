...
- серверные ограничения очереди по часу, дню, домену получателя и интервалу;
...
`artel.office8@gmail.com` подключается через OAuth 2.0-интеграцию Gmail API. Очередь
предназначена только для предварительно проверенных компаний со статусом «Готова» и
не отменяет требования к релевантности обращения и обработке отказов получателей.
...
попадают только действующие компании со статусом «Готова», которым ещё не отправляли
...
По умолчанию backend применяет консервативную политику:
...
автоматически. Любая ошибка Gmail также ставит очередь на паузу. Достигнутый часовой или
дневной лимит не вызывает частое повторение запросов: следующее письмо планируется на
разрешённое время.
...
«Готова». Низкая скорость снижает риск технических ограничений, но не гарантирует
