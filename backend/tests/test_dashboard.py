...
def test_dashboard_returns_six_month_complete_week_history(db):
    ...
    assert len(history) == DASHBOARD_HISTORY_DAYS == 189
    assert date.fromisoformat(history[0]["date"]) == current_week_start - timedelta(weeks=26)
